# claims_denial_analyzer.py
"""
Claims Denial Reason Analyzer & Appeal Letter Drafting System
Full Python implementation with LangChain and Google Gemini
"""
...
# ============================================================================
# LANGCHAIN COMPONENTS WITH GEMINI
# ============================================================================

class DenialAnalyzer:
    """Core LangChain-based denial analysis engine using Google Gemini"""
    ...
    def __init__(self, api_key: str, model_name: str = "gemini-2.5-flash"):
        ...
        Args:
            ...
            model_name: Gemini model name (gemini-2.5-flash, gemini-2.5-pro, gemini-2.0-flash-exp)
        """
...
class AppealLetterGenerator:
    """LangChain-based appeal letter generator using Google Gemini"""
...
class PDFClaimExtractor:
    """Extract structured claim information from denial letter PDFs using Gemini AI"""
...
def main():
    st.set_page_config(
        page_title="Claims Denial Analyzer",
        page_icon="📋",
        layout="wide"
    )
    ...
    with st.sidebar:
        ...
        # st.metric("Claims Analyzed Today", "47")
        # st.metric("Appeals Generated", "32")
        ...
        st.info(
            "This tool uses LangChain and Google Gemini to analyze "
    ...
    st.markdown(
        "AI-powered denial analysis and appeal generation for healthcare revenue cycle teams"
    )
        ...
        if not st.session_state.pdf_uploaded:
            with st.spinner("📖 Extracting text from PDF..."):
                ...
                if pdf_text:
                    ...
                    if api_key:
                        with st.spinner("🤖 Extracting claim information with AI..."):
                            try:
                        ...
                        st.warning("⚠️ Please enter your Google API key in the sidebar to auto-extract claim information.")
                else:
    ...
    if st.session_state.pdf_uploaded:
        ...
        if st.button("🧠 Analyze Denial with AI", type="primary", use_container_width=True):
            if not api_key:
                st.error("⚠️ Google API key not found in .env file. Please add GOOGLE_API_KEY to your .env file.")
            elif not denial_text:
                st.error("⚠️ Please provide at least the denial text to analyze")
            else:
                with st.spinner("🔄 Analyzing denial with Gemini AI..."):
                    try:
                        # Create claim data
                        ...
                        st.info("💡 Tip: Make sure your API key is valid and you have sufficient quota.")
    ...
    if st.session_state.analysis is not None:
        ...
        if st.session_state.appeal_letter is None:
            with st.spinner("📝 Generating appeal letter with Gemini..."):
                try:
